...
class MLP:
    ...
    #本次作業新增
    def dydx(self, x):
        y, (x_, z1, a1, z2, a2, z3) = self.forward(x, cache=True)
        delta3 = np.ones_like(y)  
        # layer 2 and 1 
        delta2 = (delta3 @ self.W3.T) * self.tanh_prime(z2)   # (N,h2)
        delta1 = (delta2 @ self.W2.T) * self.tanh_prime(z1)   # (N,h1)

        dydx_vals = (delta1 @ self.W1.T)     
        return dydx_vals
...
if __name__ == "__main__":
    ...
    y_true     = runge(x_grid)
    dy_pred    = net.dydx(x_grid)
    dy_true    = runge_prime(x_grid)

    mse_f      = np.mean((y_pred - y_true)**2)
    maxerr_f   = np.max(np.abs(y_pred - y_true))
    mse_df     = np.mean((dy_pred - dy_true)**2)
    maxerr_df  = np.max(np.abs(dy_pred - dy_true))

    print(f"MSE(f):     {mse_f:.6e}   MaxErr(f):  {maxerr_f:.6e}")
    print(f"MSE(f') :   {mse_df:.6e}  MaxErr(f'): {maxerr_df:.6e}")
    ...
    plt.title("Runge function approximation (value)")
    plt.legend(); plt.tight_layout(); plt.show()

    plt.figure()
    plt.plot(x_grid[:,0], dy_true[:,0], label="True f'(x)")
